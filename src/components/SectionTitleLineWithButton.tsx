...
import { currentPageTitleMap } from "../config"
type Props = {
  icon: string
  title: string
  main?: boolean
...
export default function SectionTitleLineWithButton({ icon, title, main = false, children }: Props) {
  ...
  return (
    <section style={{ fontFamily: 'Vazir' }} className={`${main ? '' : 'pt-6'} mb-6 flex items-center justify-between `} >
      <div className="flex items-center justify-start">
        ...
        <h1 className={`leading-tight ${main ? 'text-3xl' : 'text-2xl'}`}>{currentPageTitleMap[title] || title}</h1>
      </div>
      {children}
