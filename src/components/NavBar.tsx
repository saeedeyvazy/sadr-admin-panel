...
export default function NavBar({ menu, className = '', children }: Props) {
  ...
  return (
    ...
    >
      <div className={`flex flex-row-reverse lg:items-stretch  ${containerMaxW}`}>
        <div className="flex flex-1 items-stretch h-14">{children}</div>
        <div
          className={`${isMenuNavBarActive ? 'block' : 'hidden'
            } max-h-screen-menu overflow-y-auto whitespace-nowrap lg:overflow-visible absolute w-screen top-14 left-0 bg-gray-50 shadow-lg lg:w-auto lg:flex lg:static lg:shadow-none dark:bg-slate-800`}
        >
